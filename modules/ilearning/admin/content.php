    ...
    if (empty($array['title'])) {
        ...
        if (empty($error)) {
            ...
            if (empty($id) or $is_copy) {
                ...
                if($_FILES['extfile']['name']){
                   $filename = NV_ROOTDIR . '/' . NV_TEMP_DIR . '/' . NV_TEMPNAM_PREFIX . 'auto1_' . NV_CHECK_SESSION . '.zip';
                ...
                $folder = 'folder_' . NV_CURRENTTIME;
                $extract = $zip->extract(PCLZIP_OPT_PATH, NV_ROOTDIR . '/' . NV_UPLOADS_DIR . '/' . $module_upload . '/' . $folder);
            }
            ...
            $sql = "INSERT INTO " . NV_MOD_TABLE . " (folder,
        ...
        if($_FILES['extfile']['name']){
            $folder = $db->query('SELECT folder FROM ' . NV_MOD_TABLE . ' WHERE id = ' . $id)->fetchColumn();
            remove_dir($folder);
            $filename = NV_ROOTDIR . '/' . NV_TEMP_DIR . '/' . NV_TEMPNAM_PREFIX . 'auto1_' . NV_CHECK_SESSION . '.zip';
            if (!isset($_FILES, $_FILES['extfile'], $_FILES['extfile']['tmp_name'])) {
                $error = $lang_module['autoinstall_error_downloaded'];
...
$xtpl->assign('ACTION', NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op . '&amp;upload=1');
$xtpl->assign('LANG', $lang_module);
...
$xtpl->assign('SUBMIT_CHECKSESS', md5(NV_CHECK_SESSION . 'submit-ext1'));
$num_display_field = 0;
...
if($array['folder']){
 $xtpl->assign('LINK_BAI_GIANG', NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/' . $array['folder']);
 $xtpl->parse('main.folder');
}
